...
import com.ghanaride.security.jwt.JwtAuthenticationFilter;
...
/**
 * GhanaRide Security Configuration - FIXED for Next.js frontend
 * - "/" and public pages now permitAll (fixes redirect loop)
 * - JWT filter added for /api/**
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                .cors(cors -> {}) // Uses CorsConfig bean for Next.js frontend
                .authorizeHttpRequests(auth -> auth
                        ...
                        // PUBLIC PAGES - MUST BE permitAll to avoid redirect loop on "/"
                        .requestMatchers("/", "/home", "/index", "/landing").permitAll()
                        .requestMatchers("/about", "/contact", "/faq", "/privacy", "/terms", "/refunds", "/routes").permitAll()
                        .requestMatchers("/login", "/register", "/forgot-password", "/reset-password", "/logout-success").permitAll()

                        // Public route browsing (no login needed to SEE rides)
                        .requestMatchers(HttpMethod.GET, "/rides", "/rides/**", "/routes/**").permitAll()
                        ...
                        // API endpoints - Public trips search, Auth public, rest authenticated
                        .requestMatchers(HttpMethod.GET, "/api/trips/**").permitAll()
                        .requestMatchers(HttpMethod.GET, "/api/trips/search").permitAll()
                        .requestMatchers("/api/auth/login", "/api/auth/register", "/api/auth/refresh", "/api/auth/forgot-password", "/api/auth/reset-password", "/api/auth/oauth/**").permitAll()
        ...
        // JWT filter for API + Rate limit filter
    ...
    private String buildCspPolicy() {
        return String.join("; ",
                ...
                "connect-src 'self' https://api.paystack.co https://accounts.google.com wss://ghanaride.me http://localhost:3000 http://localhost:8088",
